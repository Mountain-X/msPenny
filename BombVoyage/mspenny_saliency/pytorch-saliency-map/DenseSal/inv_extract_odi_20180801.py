...
class Setup_extract_omni_image():
    ...
    def __init__(self, extract_num = 6, extract_outputsize = 400,
        ...
        self.mask = np.load('mask.npz')['mask']
...
#画像を入力
omniData = np.array(Image.open('105.png'))

#抽出
omniSetupper = Setup_extract_omni_image(elevation_angle = 30)
...
salMaps[0] = np.array(Image.open('planar_30_0.png').resize((frames[0].shape[0], frames[0].shape[1])))
salMaps[1] = np.array(Image.open('planar_30_1.png').resize((frames[0].shape[0], frames[0].shape[1])))
salMaps[2] = np.array(Image.open('planar_30_2.png').resize((frames[0].shape[0], frames[0].shape[1])))
salMaps[3] = np.array(Image.open('planar_30_3.png').resize((frames[0].shape[0], frames[0].shape[1])))
salMaps[4] = np.array(Image.open('planar_30_4.png').resize((frames[0].shape[0], frames[0].shape[1])))
salMaps[5] = np.array(Image.open('planar_30_5.png').resize((frames[0].shape[0], frames[0].shape[1])))
#salMaps = np.ones_like(frames[:,:,:,0])*255
...
#逆変換
time0 = time.time()
dualFishEyeSalMap, time1, tmp = inverse_extract_omni_image(time0, salMaps, omniData, omniSetupper, overlaid_weight1 = 0.5, overlaid_weight2 = 0.7)
